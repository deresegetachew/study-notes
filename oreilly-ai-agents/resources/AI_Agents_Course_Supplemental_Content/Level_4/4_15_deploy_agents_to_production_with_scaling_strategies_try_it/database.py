"""
SQLite Database Operations with Multi-User Support

This module handles all database operations for storing and retrieving
long-running task states and results, scoped to individual users.

Key changes from 4_14:
- Added user_id column to tasks table
- Added get_user_tasks() to list tasks by user
- All task operations now validate user ownership
"""

import os
import sqlite3
import json
from datetime import datetime
from typing import Optional
from contextlib import contextmanager

# Database path - configurable via environment variable for Docker
DATABASE_PATH = os.getenv("DATABASE_PATH", "tasks.db")


@contextmanager
def get_connection():
    """Context manager for database connections."""
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    conn.row_factory = sqlite3.Row
    try:
        yield conn
    finally:
        conn.close()


def init_db():
    """Initialize the database and create tables if they don't exist."""
    with get_connection() as conn:
        # Tasks table with user_id for multi-tenant support
        conn.execute("""
            CREATE TABLE IF NOT EXISTS tasks (
                task_id TEXT PRIMARY KEY,
                user_id TEXT NOT NULL,
                query TEXT NOT NULL,
                conversation_id TEXT,
                status TEXT DEFAULT 'pending',
                progress TEXT,
                execution_strategy TEXT,
                result TEXT,
                error TEXT,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        """)

        # Index for fast user task lookups
        conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_tasks_user_id
            ON tasks(user_id)
        """)

        # Index for conversation-based lookups
        conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_tasks_conversation
            ON tasks(user_id, conversation_id)
        """)

        conn.commit()


def create_task(
    task_id: str,
    user_id: str,
    query: str,
    conversation_id: Optional[str] = None
) -> dict:
    """
    Create a new task entry in the database.

    Args:
        task_id: Unique identifier for the task
        user_id: ID of the user who submitted the task
        query: The user's query to process
        conversation_id: Optional conversation context

    Returns:
        Dictionary with task_id and initial status
    """
    with get_connection() as conn:
        conn.execute(
            """
            INSERT INTO tasks (task_id, user_id, query, conversation_id, status, created_at, updated_at)
            VALUES (?, ?, ?, ?, 'pending', ?, ?)
            """,
            (task_id, user_id, query, conversation_id, datetime.now(), datetime.now())
        )
        conn.commit()

    return {"task_id": task_id, "user_id": user_id, "status": "pending"}


def update_task_status(
    task_id: str,
    status: str,
    progress: Optional[str] = None,
    execution_strategy: Optional[str] = None
):
    """
    Update the status and progress of a task.

    Args:
        task_id: Task identifier
        status: New status (pending, running, completed, failed)
        progress: Optional progress description
        execution_strategy: Optional execution strategy (sequential, parallel)
    """
    with get_connection() as conn:
        if execution_strategy:
            conn.execute(
                """
                UPDATE tasks
                SET status = ?, progress = ?, execution_strategy = ?, updated_at = ?
                WHERE task_id = ?
                """,
                (status, progress, execution_strategy, datetime.now(), task_id)
            )
        else:
            conn.execute(
                """
                UPDATE tasks
                SET status = ?, progress = ?, updated_at = ?
                WHERE task_id = ?
                """,
                (status, progress, datetime.now(), task_id)
            )
        conn.commit()


def save_task_result(task_id: str, result: dict):
    """
    Save the final result of a completed task.

    Args:
        task_id: Task identifier
        result: Dictionary containing the task result
    """
    with get_connection() as conn:
        conn.execute(
            """
            UPDATE tasks
            SET status = 'completed', result = ?, updated_at = ?
            WHERE task_id = ?
            """,
            (json.dumps(result), datetime.now(), task_id)
        )
        conn.commit()


def save_task_error(task_id: str, error: str):
    """
    Save error information for a failed task.

    Args:
        task_id: Task identifier
        error: Error message
    """
    with get_connection() as conn:
        conn.execute(
            """
            UPDATE tasks
            SET status = 'failed', error = ?, updated_at = ?
            WHERE task_id = ?
            """,
            (error, datetime.now(), task_id)
        )
        conn.commit()


def get_task(task_id: str, user_id: Optional[str] = None) -> Optional[dict]:
    """
    Retrieve a task by its ID, optionally validating user ownership.

    Args:
        task_id: Task identifier
        user_id: Optional user ID for ownership validation

    Returns:
        Task dictionary or None if not found (or not owned by user)
    """
    with get_connection() as conn:
        if user_id:
            cursor = conn.execute(
                "SELECT * FROM tasks WHERE task_id = ? AND user_id = ?",
                (task_id, user_id)
            )
        else:
            cursor = conn.execute(
                "SELECT * FROM tasks WHERE task_id = ?",
                (task_id,)
            )
        row = cursor.fetchone()

        if row is None:
            return None

        return {
            "task_id": row["task_id"],
            "user_id": row["user_id"],
            "query": row["query"],
            "conversation_id": row["conversation_id"],
            "status": row["status"],
            "progress": row["progress"],
            "execution_strategy": row["execution_strategy"],
            "error": row["error"],
            "created_at": row["created_at"],
            "updated_at": row["updated_at"]
        }


def get_task_result(task_id: str, user_id: Optional[str] = None) -> Optional[dict]:
    """
    Retrieve the result of a completed task.

    Args:
        task_id: Task identifier
        user_id: Optional user ID for ownership validation

    Returns:
        Result dictionary or None if not found/not completed
    """
    with get_connection() as conn:
        if user_id:
            cursor = conn.execute(
                "SELECT result, status FROM tasks WHERE task_id = ? AND user_id = ?",
                (task_id, user_id)
            )
        else:
            cursor = conn.execute(
                "SELECT result, status FROM tasks WHERE task_id = ?",
                (task_id,)
            )
        row = cursor.fetchone()

        if row is None:
            return None

        if row["status"] != "completed":
            return None

        if row["result"]:
            return json.loads(row["result"])

        return None


def get_user_tasks(
    user_id: str,
    limit: int = 50,
    offset: int = 0,
    status_filter: Optional[str] = None
) -> list[dict]:
    """
    Get all tasks for a specific user.

    Args:
        user_id: User identifier
        limit: Maximum number of tasks to return
        offset: Pagination offset
        status_filter: Optional status to filter by

    Returns:
        List of task dictionaries
    """
    with get_connection() as conn:
        if status_filter:
            cursor = conn.execute(
                """
                SELECT task_id, user_id, query, conversation_id, status,
                       progress, execution_strategy, error, created_at, updated_at
                FROM tasks
                WHERE user_id = ? AND status = ?
                ORDER BY created_at DESC
                LIMIT ? OFFSET ?
                """,
                (user_id, status_filter, limit, offset)
            )
        else:
            cursor = conn.execute(
                """
                SELECT task_id, user_id, query, conversation_id, status,
                       progress, execution_strategy, error, created_at, updated_at
                FROM tasks
                WHERE user_id = ?
                ORDER BY created_at DESC
                LIMIT ? OFFSET ?
                """,
                (user_id, limit, offset)
            )

        rows = cursor.fetchall()

        return [
            {
                "task_id": row["task_id"],
                "user_id": row["user_id"],
                "query": row["query"],
                "conversation_id": row["conversation_id"],
                "status": row["status"],
                "progress": row["progress"],
                "execution_strategy": row["execution_strategy"],
                "error": row["error"],
                "created_at": row["created_at"],
                "updated_at": row["updated_at"]
            }
            for row in rows
        ]


def get_conversation_tasks(user_id: str, conversation_id: str) -> list[dict]:
    """
    Get all tasks for a specific conversation.

    Args:
        user_id: User identifier
        conversation_id: Conversation identifier

    Returns:
        List of task dictionaries in chronological order
    """
    with get_connection() as conn:
        cursor = conn.execute(
            """
            SELECT task_id, query, status, execution_strategy, created_at
            FROM tasks
            WHERE user_id = ? AND conversation_id = ?
            ORDER BY created_at ASC
            """,
            (user_id, conversation_id)
        )

        return [
            {
                "task_id": row["task_id"],
                "query": row["query"],
                "status": row["status"],
                "execution_strategy": row["execution_strategy"],
                "created_at": row["created_at"]
            }
            for row in cursor.fetchall()
        ]
