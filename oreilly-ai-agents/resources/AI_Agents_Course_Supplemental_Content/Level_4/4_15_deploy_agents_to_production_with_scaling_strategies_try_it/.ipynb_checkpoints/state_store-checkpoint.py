...
HINTS:
    ...
    - Use EXPIRE to set TTL on first request in window
    - Use GET to check current count
    - Time windows can be: current minute, or rolling window
...
def check_rate_limit(user_id: str) -> Tuple[bool, int, int]:
    ...
    HINTS:
    1. Get the Redis client: client = get_redis_client()
    2. Generate the rate limit key: key = _rate_limit_key(user_id)
    3. Get current count: current_count = client.get(key)
    4. If None, user hasn't made requests this window: current_count = 0
    5. Convert to int: current_count = int(current_count)
    6. Check if under limit: is_allowed = current_count < RATE_LIMIT_MAX_REQUESTS
    7. Return: (is_allowed, current_count, RATE_LIMIT_MAX_REQUESTS)
...
def increment_rate_limit(user_id: str) -> int:
    ...
    1. Get the Redis client: client = get_redis_client()
    2. Generate the rate limit key: key = _rate_limit_key(user_id)
    3. Increment counter: new_count = client.incr(key)
       - INCR creates the key with value 1 if it doesn't exist
       - INCR atomically increments if it exists
    4. Set TTL on first request: if new_count == 1: client.expire(key, RATE_LIMIT_WINDOW_SECONDS)
       - This ensures the key auto-deletes after the window
    5. Return: new_count
...
def get_rate_limit_status(user_id: str) -> dict:
    ...
    1. Get the Redis client: client = get_redis_client()
    2. Generate the rate limit key: key = _rate_limit_key(user_id)
    3. Get current count: current_count = client.get(key) or 0
    4. Get TTL (time to live): ttl = client.ttl(key)
       - Returns -2 if key doesn't exist
       - Returns -1 if key exists but has no TTL
       - Returns positive number = seconds remaining
    5. Calculate remaining: remaining = max(0, RATE_LIMIT_MAX_REQUESTS - int(current_count))
    6. Handle TTL edge cases: reset_seconds = ttl if ttl > 0 else RATE_LIMIT_WINDOW_SECONDS
