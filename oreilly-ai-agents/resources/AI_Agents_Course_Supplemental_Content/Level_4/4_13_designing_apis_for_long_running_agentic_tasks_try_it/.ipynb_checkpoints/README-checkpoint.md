...
Implement the `run_with_streaming()` function at line ~120.

**Requirements:**
1. Submit task using existing `submit_task()` function
2. Connect to `/tasks/{task_id}/stream` with `stream=True`
3. Create an SSE client and iterate through events
...
├── README.md              # This file
├── requirements.txt       # Dependencies (includes SSE libraries)
...
├── client.py             # Client with TODO for streaming mode
